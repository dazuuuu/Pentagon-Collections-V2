...
<head>
  ...
  <title>Admin Login | Al NAHDA Agency</title>
...
<body class="bg-[#0f2038] text-white antialiased min-h-screen flex items-center justify-center p-4">
  <div class="w-full max-w-sm min-w-0">
    <div class="text-center mb-8">
      ...
      <h1 class="font-serif-heading text-2xl font-bold tracking-widest uppercase">Al NAHDA Admin</h1>
      <p class="text-xs text-amber-200/60 mt-1">Sign in to manage applications &amp; respond to applicants</p>
    </div>

    <form method="post" action="<?= url('/admin/login') ?>" class="bg-[#132c5c] border border-amber-500/20 rounded-xl p-6 space-y-4 shadow-2xl">
      <?= csrfField() ?>
      <?php if (!empty($_SESSION['flash_success'])): ?>
        <div class="bg-emerald-950 border border-emerald-600/40 text-emerald-300 text-xs font-semibold rounded-lg p-3"><?= e($_SESSION['flash_success']) ?></div>
      ...
      <div>
        ...
        <input type="email" name="email" required autofocus class="w-full mt-1 bg-[#0f2038] border border-amber-500/30 text-white text-sm rounded-lg px-3 py-2.5 focus:outline-none focus:border-amber-400" />
      </div>
      <div>
        <div class="flex items-center justify-between">
          <label class="text-[11px] font-bold text-amber-200/80 uppercase tracking-wider">Password</label>
          <a href="<?= url('/admin/forgot-password') ?>" class="text-[11px] font-semibold text-amber-300/80 hover:text-amber-300">Forgot password?</a>
        </div>
        <input type="password" name="password" required class="w-full mt-1 bg-[#0f2038] border border-amber-500/30 text-white text-sm rounded-lg px-3 py-2.5 focus:outline-none focus:border-amber-400" />
      </div>
    ...
    <p class="text-center text-[11px] text-amber-200/40 mt-6">
      <a href="<?= url('/') ?>" class="hover:text-amber-300">&larr; Back to website</a>
    </p>
  </div>
</body>
</html>
